...
import uk.ac.ucl.gui.dialogs.ChartDialog;
...
public class GUI {
    ...
    private void createButtons(){
        ...
        graphButton = new JButton("Charts");
        graphButton.addActionListener(event -> chartData());
    }
    ...
    private void loadData(){
        String filename = getFileName(frame, false);
    ...
    private void findData() {
        FindDialog findDialog = new FindDialog(frame, controller.getColumnNames());
        if (findDialog.getResponse() == FindDialog.SUBMITTED) {
            if (findDialog.getSearchType() == Find.STRING) {
                displayMatchedStrings(findDialog.getColumnIndex(), findDialog.getSearchPhrase(), findDialog.getOperation());
            } else {
    ...
    private void exportData(){
        String filename = getFileName(frame, true);
    ...
    private void chartData(){
        try {
            Chart chart = ChartDialog.getChart(frame, controller);
            if (chart != null) new ChartDisplay(chart); // if null then don't display anything.
        ...
        } catch (TooManyValuesUnableToChart exception) {
            JOptionPane.showMessageDialog(frame, "Unable to place all the values (30+)","Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void displayMatchedStrings(int columnIndex, String text, int operation){
        if (text == null) return;
        controller.configureRowsToMatch(columnIndex, text, operation);
        rebuildTable();
    ...
    private void displayExtremeData(int columnIndex, int searchType, int operation){
        try {
            ...
            JOptionPane.showMessageDialog(frame, exception.exceptionAt() + " doesn't fit the format of a date.",
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private static String getFileName(JFrame frame, boolean save){
