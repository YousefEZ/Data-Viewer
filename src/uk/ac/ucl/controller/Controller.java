...
public class Controller {
    ...
    public void configureRowsToMatch(int columnIndex, String text, int operation){
        displayedRows.removeAll(searchStrings(columnIndex, text, operation));
    }
    ...
    private int matchSearchType(int columnIndex, int searchType, int operation) throws InvalidDateStringException {
        switch (searchType){
            ...
            default:
                throw new IllegalStateException("Unexpected Search Type Value: " + searchType);
        }
    }
    ...
    private int searchNumber(int columnIndex, int operation){
        ...
        if (operation == Find.LARGEST) {
            return Find.findLargestNumberIndex(columnData, displayedRows);
        } else if (operation == Find.SMALLEST) {
            return Find.findSmallestNumberIndex(columnData, displayedRows);
        }
    ...
    private int searchYear(int columnIndex, int operation) throws InvalidDateStringException {
        ...
        if (operation == Find.NEWEST){
            return Find.findNewestIndex(columnData, displayedRows);
        } else if (operation == Find.OLDEST){
            return Find.findOldestIndex(columnData, displayedRows);
        }
        throw new IllegalStateException("Unexpected Operation Value: " + operation);
    }

    // map the operation to the method in the Find helper class.
    private List<Integer> searchStrings(int columnIndex, String text, int operation) {
        List<String> columnData = model.getColumnData(columnIndex);

        if (operation == Find.CONTAINS){
            return Find.findStringsNotContainingString(columnData, displayedRows, text);
        } else if (operation == Find.MATCHES){
            return Find.findUnmatchedStringsIndices(columnData, displayedRows, text);
        }
        throw new IllegalStateException("Unexpected Operation Value: " + operation);
    }
